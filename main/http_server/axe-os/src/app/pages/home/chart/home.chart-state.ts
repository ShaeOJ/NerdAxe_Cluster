...
export class HomeChartState {
  ...
  validateLengthsOrReset(): boolean {
    ...
    this.clusterHashrate = normalizeToLen(this.clusterHashrate, lenLabels);
    return true;
  }
...
private trimToCutoff(cutoffMs: number): void {
  ...
  this.clusterHashrate = this.clusterHashrate.slice(idx);
...
trimToWindow(nowMs: number, windowMs: number = HOME_CFG.xAxis.fixedWindowMs): void {
  ...
  toPersisted(): PersistedHomeChartStateV1 {
    return {
      ...
      dataClusterHashrate: this.clusterHashrate,
    };
  }
  ...
  static fromPersisted(raw: unknown): HomeChartState {
    ...
    const cluster = Array.isArray(parsed?.dataClusterHashrate) ? parsed.dataClusterHashrate.map(Number) : null;
    ...
    s.clusterHashrate = normalizeToLen(cluster, targetLen);
  ...
  appendHistoryChunk(chunk: ApiHistoryChunk, opts: AppendHistoryOptions = {}): number {
    ...
    const cluster = Array.isArray(chunk?.cluster_hashrate) ? (chunk.cluster_hashrate as any[]).map(Number) : [];
    ...
    for (let i = 0; i < n; i++) {
      ...
      if (lastIdx >= 0 && this.labels[lastIdx] === tsAbs) {
        ...
        this.clusterHashrate[lastIdx] = (cluster.length && cluster[i] > 0) ? cluster[i] * 1000000000.0 / 100.0 : NaN;
        continue;
      }
      ...
      this.clusterHashrate.push((cluster.length && cluster[i] > 0) ? cluster[i] * 1000000000.0 / 100.0 : NaN);
      appended++;
    }

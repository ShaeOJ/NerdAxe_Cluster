...
import { catchError, interval, map, Observable, of, Subscription, firstValueFrom, startWith, switchMap } from 'rxjs';
...
import { IClusterStatus } from '../../models/IClusterStatus';
import {
...
export class HomeExperimentalComponent implements AfterViewChecked, OnInit, OnDestroy {
  ...
  public get dataClusterTotal(): number[] { return this.chartState.clusterHashrate; }
  public set dataClusterTotal(v: number[]) { this.chartState.clusterHashrate = v; }
  ...
  public isClusterSlave: boolean = false;
  public isClusterMaster: boolean = false;
  ...
  constructor(
    ...
    const cfg = createHomeChartConfig({
      series: {
        ...
        clusterTotal: this.dataClusterTotal,
      },
    ...
    // Fetch cluster status periodically to detect slave mode
    interval(5000).pipe(
      startWith(0),
      switchMap(() => this.systemService.getSwarmInfo().pipe(
        map(res => res as any as IClusterStatus),
        catchError(() => of(null)),
      )),
    ).subscribe(status => {
      if (status === null) return;
      this.clusterStatus = status;
      this.isClusterSlave = status?.mode === 'slave';
      this.isClusterMaster = status?.mode === 'master';
    });
  }
...
ngOnInit() {
  ...
  private getPoolHashrateHsSum(): number {
    // Hashrate for pill (and any live reference): from pool sums, converted to H/s.
    // getPoolHashrate() returns GH/s, while the chart series values are in H/s.
    // In slave mode, pools aren't connected — fall back to raw info.hashRate.
    try {
      const a = Number(this.getPoolHashrate(0));
      const b = Number(this.getPoolHashrate(1));
      let sumGh = (Number.isFinite(a) ? a : 0) + (Number.isFinite(b) ? b : 0);
      // Slave mode fallback: pool sum is 0 because stratum isn't directly connected.
      // Use the device's own reported hashrate instead so warmup/startup logic works.
      if ((!Number.isFinite(sumGh) || sumGh <= 0) && this._info?.hashRate) {
        sumGh = Number(this._info.hashRate);
      }
...
private setAxisPadding(cfg: any, persist: boolean = false): void {
  ...
  private insertHardBreakSample(breakAtMs: number): void {
    ...
    this.dataClusterTotal.push(NaN);
  ...
  private updateChartData(data: any): void {
    ...
    const clusterRaw: number[] = Array.isArray(data.cluster_hashrate) ? data.cluster_hashrate : [];
    ...
    for (let i = 0; i < n; i++) {
      ...
      const clusterVal = clusterRaw.length > i ? Number(clusterRaw[i]) : 0;
      newData.push({
        timestamp: tsAbs,
        ...
        clusterTotal: clusterVal > 0 ? clusterVal * 1000000000.0 / 100.0 : NaN,
      });
    }
    ...
    for (const entry of newData) {
      ...
      if (lastIdx >= 0 && this.dataLabel[lastIdx] === entry.timestamp) {
        ...
        this.dataClusterTotal[lastIdx] = entry.clusterTotal ?? NaN;
        continue;
      }
      ...
      this.dataClusterTotal.push(entry.clusterTotal ?? NaN);
    }
  }
...
private updateTempScaleFromLatest(): void {
  ...
  private updateChart() {
    ...
    this.chartData.datasets[6].data = this.dataClusterTotal;
  ...
  private clearChartHistoryInternal(updateChartNow: boolean): void {
    ...
    // Allow full 1-hour history to be fetched on next poll (no timestamp restriction).
  ...
  public clusterSessionBestDiff(info: ISystemInfo): number {
    const masterBest = info?.stratum?.pools?.[0]?.bestDiff ?? 0;
    const slaveBest  = this.clusterStatus?.bestDiff ?? 0;
    return Math.max(masterBest, slaveBest);
  }

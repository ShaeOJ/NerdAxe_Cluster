...
export class HomeComponent implements AfterViewChecked, OnInit, OnDestroy {
  ...
  public dataClusterTotal: number[] = [];
    ...
    this.chartData = {
      ...
      datasets: [
        ...
        },
        {
          type: 'line',
          label: 'Cluster Total',
          data: this.dataClusterTotal,
          yAxisID: 'y',
          fill: false,
          backgroundColor: 'rgba(0,220,120,0)',
          borderColor: '#00DC78',
          tension: 0.5,
          pointRadius: 0,
          borderWidth: 2.2,
  ...
  private updateChartData(data: any): void {
    ...
    const convertedClusterTotal = Array.isArray(data.cluster_hashrate)
      ? data.cluster_hashrate.map((hr: number) => hr > 0 ? hr * 1000000000.0 / 100.0 : NaN)
      : data.timestamps.map(() => NaN);
    ...
    const newData = convertedTimestamps.map((ts, index) => ({
      ...
      clusterTotal: convertedClusterTotal[index],
    ...
    if (newData.length > 0) {
      ...
      this.dataClusterTotal = [...this.dataClusterTotal, ...newData.map(entry => entry.clusterTotal)];
    }
  }
  ...
  private loadChartData(): void {
    ...
    if (storedData) {
      ...
      this.dataClusterTotal = parsedData.dataClusterTotal || [];
    }
  ...
  private saveChartData(): void {
    ...
    const dataToSave = {
      ...
      dataClusterTotal: this.dataClusterTotal,
    };
  ...
  private validateOrResetHistory() {
    ...
    const lenClusterTotal = this.dataClusterTotal.length;

    const lengths = [lenLabels, len1m, len10m, len1h, len1d, lenVregTemp, lenAsicTemp, lenClusterTotal];
  ...
  /**
   * Returns the best difficulty seen cluster-wide since boot.
   * Combines the master's own pool session best with the best diff reported by
   * slave shares, so the displayed number reflects all workers in the cluster.
   */

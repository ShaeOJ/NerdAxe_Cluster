...
@Component({
  ...
  styles: [
    `
      ...
      .gauge-value {
        fill: none;
        stroke: var(--gauge-stroke, var(--pipboy-accent, #2d8ad7));
        stroke-width: 14;
...
export class GaugeComponent {
  ...
  @Input() okColor: string = '';   // empty = use CSS accent color
  ...
  get strokeColor(): string {
    switch (this.level) {
      ...
      default: {
        if (this.okColor) return this.okColor;
        // Read accent color from CSS custom property
        const accent = getComputedStyle(document.documentElement).getPropertyValue('--pipboy-accent').trim();
        return accent || '#2d8ad7';
      }
    }
  }
}

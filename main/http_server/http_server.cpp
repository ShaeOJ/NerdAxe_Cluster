...
esp_err_t start_rest_server(void * pvParameters)
{
    ...
    // Per-slave restart — register before the wildcard PATCH so it matches first
    httpd_uri_t swarm_slave_restart_uri = {
        .uri = "/api/swarm/slaves/*/restart",
        .method = HTTP_POST,
        .handler = POST_swarm_slave_restart,
        .user_ctx = rest_context
    };
    httpd_register_uri_handler(http_server, &swarm_slave_restart_uri);

    httpd_uri_t swarm_slave_restart_options_uri = {
        .uri = "/api/swarm/slaves/*/restart",
        .method = HTTP_OPTIONS,
        .handler = handle_options_request,
        .user_ctx = NULL
    };
    httpd_register_uri_handler(http_server, &swarm_slave_restart_options_uri);

    // Per-slave config
    httpd_uri_t swarm_slave_patch_uri = {
        .uri = "/api/swarm/slaves/*",
        .method = HTTP_PATCH,
        .handler = PATCH_swarm_slave,
        .user_ctx = rest_context
    };
    httpd_register_uri_handler(http_server, &swarm_slave_patch_uri);

    httpd_uri_t swarm_slave_options_uri = {
        .uri = "/api/swarm/slaves/*",
        .method = HTTP_OPTIONS,
        .handler = handle_options_request,
        .user_ctx = NULL
    };
    httpd_register_uri_handler(http_server, &swarm_slave_options_uri);

    // Watchdog status + enable/disable
    httpd_uri_t swarm_watchdog_get_uri = {
        .uri = "/api/swarm/watchdog", .method = HTTP_GET, .handler = GET_swarm_watchdog, .user_ctx = rest_context};
    httpd_register_uri_handler(http_server, &swarm_watchdog_get_uri);

    httpd_uri_t swarm_watchdog_post_uri = {
        .uri = "/api/swarm/watchdog", .method = HTTP_POST, .handler = POST_swarm_watchdog, .user_ctx = rest_context};
    httpd_register_uri_handler(http_server, &swarm_watchdog_post_uri);

    httpd_uri_t swarm_watchdog_options_uri = {
        .uri = "/api/swarm/watchdog", .method = HTTP_OPTIONS, .handler = handle_options_request, .user_ctx = NULL};
    httpd_register_uri_handler(http_server, &swarm_watchdog_options_uri);

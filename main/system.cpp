...
void System::pushHistory() {
    ...
    float clusterTotal = cluster_integration_is_master()
        ? cluster_integration_get_total_hashrate()
        : 0.0f;
    m_history->push(hashrate, filteredVreg, filteredAsicTemp, timestamp, clusterTotal);
}

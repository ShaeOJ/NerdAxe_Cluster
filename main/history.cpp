...
float History::getClusterHashrateSample(int index)
{
    return m_clusterHashrate ? m_clusterHashrate[WRAP(index)] : 0.0f;
}
...
bool History::init(int num_asics)
{
    ...
    m_clusterHashrate = (float *) CALLOC(HISTORY_MAX_SAMPLES, sizeof(float));
...
void History::push(float rateGh, float vregTemp, float asicTemp, uint64_t timestamp, float clusterHashrate)
{
    if (!isAvailable()) {
    ...
    m_clusterHashrate[WRAP(m_numSamples)] = (clusterHashrate > 0.0f) ? clusterHashrate : 0.0f;
                                ...
                                uint64_t current_timestamp, uint32_t limit)
{
    ...
    JsonArray clusterHashrateArr = json_history["cluster_hashrate"].to<JsonArray>();
    ...
    for (int i = start_index; i < start_index + num_samples; i++) {
        ...
        clusterHashrateArr.add((int) (getClusterHashrateSample(i) * 100.0f));

...
{
    {
        ...
        // In master mode, distribute work to all registered slaves
        if (cluster_integration_is_master()) {
            MiningInfo *mi = &miningInfo[pool];
            if (mi->extranonce_str && mi->current_job->ntime) {
                cluster_integration_distribute_work(
                    mi->current_job, pool,
                    mi->extranonce_str, mi->extranonce_2_len,
                    mi->version_mask, mi->active_stratum_difficulty,
                    STRATUM_MANAGER ? STRATUM_MANAGER->selectAsicDiff(pool, mi->active_stratum_difficulty) : mi->active_stratum_difficulty);
            }
        }
    }
...
void cluster_retrigger_work_distribution(void)
{
    if (!cluster_integration_is_master()) return;

    // Increment tick each call so every re-broadcast gives slaves a fresh extranonce2 space.
    // Using 0 for the initial mining.notify distribution; retriggers start at 1 and increment.
    static uint32_t s_retrigger_en2_tick = 1;
    uint32_t tick = s_retrigger_en2_tick++;

    PThreadGuard g(current_stratum_job_mutex);
    for (int pool = 0; pool < 2; pool++) {
        MiningInfo *mi = &miningInfo[pool];
        if (mi->extranonce_str && mi->current_job && mi->current_job->ntime) {
            ESP_LOGI(TAG, "Re-distributing work to cluster slaves (pool %d, en2_tick=%lu)", pool, (unsigned long)tick);
            uint32_t asic_diff = STRATUM_MANAGER
                ? STRATUM_MANAGER->selectAsicDiff(pool, mi->active_stratum_difficulty)
                : mi->active_stratum_difficulty;
            cluster_integration_distribute_work(
                mi->current_job, pool,
                mi->extranonce_str, mi->extranonce_2_len,
                mi->version_mask, mi->active_stratum_difficulty,
...
void *create_jobs_task(void *pvParameters)
{
    ...
    // Auto-timing only runs on master/standalone (not on slaves which have no pool connection)
    if (!cluster_integration_is_slave()) {
        auto_timing_init();
        auto_timing_start();
    }
    ...
    while (1) {
        ...
        // job interval: use auto_timing when active, otherwise fall back to board setting
        int desiredInterval = auto_timing_is_active()
            ? (int)auto_timing_get_interval_ms()
            : board->getAsicJobIntervalMs();
        if (desiredInterval != lastJobInterval) {
            xTimerChangePeriod(job_timer, pdMS_TO_TICKS(desiredInterval), 0);
            lastJobInterval = desiredInterval;
            continue;
        }
        ...
        // In master mode, refresh slave work every tick so slaves hash a fresh
        // block header (different merkle root) instead of cycling the same nonces.
        // extranonce_2 serves as the tick counter: each value produces unique
        // cluster_job_ids and extranonce2s for each slave via the en2_tick offset.
        if (cluster_integration_is_master()) {
            PThreadGuard g(current_stratum_job_mutex);
            MiningInfo *mi = &miningInfo[active_pool];
            if (mi->extranonce_str && mi->current_job && mi->current_job->ntime) {
                cluster_integration_distribute_work(
                    mi->current_job, active_pool,
                    mi->extranonce_str, mi->extranonce_2_len,
                    mi->version_mask, mi->active_stratum_difficulty,
                    STRATUM_MANAGER->selectAsicDiff(active_pool, mi->active_stratum_difficulty),
                    extranonce_2);
            }
        }

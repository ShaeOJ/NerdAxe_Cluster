...
void ASIC_result_task(void *pvParameters)
{
    ...
    while (1) {
        ...
        uint64_t bestSessionDiff = (STRATUM_MANAGER) ? STRATUM_MANAGER->getBestSessionDiff() : 0;
        suffixString(bestSessionDiff, bestDiffString, sizeof(bestDiffString), 3);
        ...
        if (duplicate) {
            ESP_LOGD(TAG, "(%s) duplicate HW nonce (ASIC returned same nonce twice, normal)", pool_str);
        ...
        if (nonce_diff >= job->pool_diff) {
            if (cluster_integration_is_slave()) {
                // In slave mode, skip duplicates — master has limited dedup buffer
                // and ESP-NOW bandwidth is constrained. Original ClusterAxe slave
                // also deduplicates before forwarding (slave_is_duplicate).
                if (!duplicate) {
                    cluster_integration_forward_share(
                        job->jobid, job->extranonce2,
                        job->ntime, asic_result.nonce,
                        asic_result.rolled_version ^ job->version,
                        job->pool_id, nonce_diff);
                }
            } else {
